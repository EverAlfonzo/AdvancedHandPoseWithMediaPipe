...
def create_video(img_dir,fourcc,video,verbose=False,fps=1):
    ...
    if osp.exists(video):
        user_confirmation = query('Output video file exists: {}\nOverwrite?'.format(video))
    if not user_confirmation:
        sys.exit()

    # Check if given image dir is valid
    if not osp.isdir(img_dir):
        raise NotADirectoryError("Input images directory is not valid: {}".format(img_dir))

    if verbose:
        print("#. Create video file from images in directory: {}".format(img_dir))
    ...
    video = cv2.VideoWriter(video, fourcc, fps, frame_shape)
