...
def hands_extraction(file_name, vid_dir, resize=(399, 299)):
    # file_name="WIFI"
    cap = cv2.VideoCapture(os.path.join(vid_dir, f'{file_name}.mp4'))
    ...
    with mp_hands.Hands(min_detection_confidence=0.8, min_tracking_confidence=0.5) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                cv2.imwrite(os.path.join(f"output/{file_name}", '{}.jpg'.format(str.rjust(str(i), 6, '0'))), img)
            else:
                i -= 1
    ...
    create_video.create_video(img_dir=file_name, fps=int(fps / 3), fourcc=fourcc, video=f"output/{file_name}.avi")
...
def main():
    hands_extraction('miercoles_ever', 'data/M')
